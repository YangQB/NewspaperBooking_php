    ...
    <body>
      ...
      <?php 
		...
		if(!isset($_SESSION['identity'])||$_SESSION['identity']!="admin"){
			...
			$result = mysql_query("SELECT BID,booking.UID,UNAME,booking.NID,NNAME,BCT,BMONTH FROM user,newspaper,booking WHERE newspaper.NID=booking.NID AND booking.UID=user.UID ORDER BY BID");
			?>
              <table class="newsp">
                <tr class='newsp_1'>
                  <th>订单编号</th>
