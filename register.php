<!DOCTYPE HTML>
  ...
  <html>
    ...
    <body>
      ...
      <div>
      <form name="form_2" method="post" onsubmit="return checkinput(this);" action="CheckRegister.php">
        <table>
          <tr>
            <td>用户账号：</td>
    ...
    <script type="text/javascript"> 
	  function checkinput(form_2)
      {
         //是否输入用户账号
         if(!isN_L(form_2.uid.value))
         {
           alert("账号格式输入错误!");
           return false;
         }
         if(!isN_L(form_2.upass.value))
         {
           alert("密码格式输入错误!");
           return false;
         }
         if(!isName(form_2.uname.value))
         {
           alert("姓名格式输入错误!");
           return false;
         }
         
         if(isIdnum(form_2.uidnum.value))
         {
           alert("身份证格式输入错误!");
           return false;
         }
         if(isPhone(form_2.uphone.value))
         {
           alert("电话号码格式输入错误!");
           return false;
         }
         if(form_2.uad.value=="null")
         {
           alert("地址格式输入错误!");
           return false;
         }
         if(form_2.upass.value != form_2.upass_1.value)
         {
